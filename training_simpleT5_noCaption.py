...
def train_step(images, instructions, target_texts, optimizer):
    ...
    attention_mask = torch.cat([query_mask, instruction_tokens.attention_mask], dim=1)
...
os.makedirs("checkpoints_t5_noCpation", exist_ok=True)
losses = []
epoch_loss_list = []
...
for epoch in range(n_epochs):
    ...
    torch.save(qformer.state_dict(), f"checkpoints_t5_noCpation/qformer_epoch{epoch+1}.pt")
    if epoch > 1:
        os.remove(f'checkpoints_t5_noCpation/qformer_epoch{epoch-1}.pt')
...
plt.legend()
plt.grid(True)
plt.savefig("checkpoints_t5_noCpation/loss_curve.png")
plt.close()
...
plt.legend()
plt.grid(True)
plt.savefig("checkpoints_t5_noCpation/loss_curve_epoch.png")
plt.close()
