...
llm = T5ForConditionalGeneration.from_pretrained("models/flan-t5-base", torch_dtype=torch.float16, device_map="auto")
...
os.makedirs("checkpoints_t5_noCaption_noIns", exist_ok=True)
losses = []
epoch_loss_list = []
...
for epoch in range(n_epochs):
    ...
    torch.save(qformer.state_dict(), f"checkpoints_t5_noCaption_noIns/qformer_epoch{epoch+1}.pt")
    if epoch > 1:
        os.remove(f'checkpoints_t5_noCaption_noIns/qformer_epoch{epoch-1}.pt')
...
plt.legend()
plt.grid(True)
plt.savefig("checkpoints_t5_noCaption_noIns/loss_curve.png")
plt.close()
...
plt.legend()
plt.grid(True)
plt.savefig("checkpoints_t5_noCaption_noIns/loss_curve_epoch.png")
plt.close()
